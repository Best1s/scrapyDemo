...
class DianpingSpider(scrapy.Spider):
    ...
    start_urls = [f"http://www.dianping.com/taiyuan/ch50/{classfiy}" for classfiy in classfiys]
    ...
    def road_parse(self,response):
        ...
        '''
        #self.verify_url(url=response.url.split("/")[2])
        pages = response.xpath('//div[@class="page"]/a[last()-1]/@data-ga-page').get()  #获取区域页面数量如果小于50 直接爬取店铺url，不用再解析每条路。
        if  pages and int(pages) < 50:
            yield scrapy.Request(url=response.url, callback=self.shop_list_parse,dont_filter=True)
        else:
