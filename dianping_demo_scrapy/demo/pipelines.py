...
class DemoPipeline(object):
    def __init__(self):         #爬虫被打开时执行 同open_spider
        print('开始爬虫'*5)
        self.file = open('dpshopurl.csv', 'a+', newline='')
...
class ShopPipeline(object):
    def __init__(self):         #爬虫被打开时执行 同open_spider
        ...
        self.file3 = open('dpshopurl.csv', 'a+', newline='')
        self.writer = csv.writer(self.file,dialect='excel')        
        self.writer2 = csv.writer(self.file2,dialect='excel')
        self.writer3 = csv.writer(self.file2,dialect='excel')
    ...
    def process_item(self, item, spider):     #当item传值过来时会被调用。
        if isinstance(item, ShopItem):
            ...
            self.writer3.writerow([item['url'],123])
        return item
